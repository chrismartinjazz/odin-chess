...
module LegalCastling
  def legal_castling_moves(color)
    moves = []
    if color == 'W'
      moves += Array(check_castling('W', 'K', 7, 1)) if @can_castle[:w_king_side]
      moves += Array(check_castling('W', 'K', 7, -1)) if @can_castle[:w_queen_side]
    else
      moves += Array(check_castling('B', 'k', 0, 1)) if @can_castle[:b_king_side]
      moves += Array(check_castling('B', 'k', 0, -1)) if @can_castle[:b_queen_side]
    end
    moves
  end
